  ...
  return (
    ...
    >
      ...
      <div className="p-4 text-zinc-700">
        <h3 className="font-semibold text-lg leading-snug mb-1.5 line-clamp">{product.name}</h3>
        <p className="text-xs text-app-text-light mt-1">{product.description}</p>

        {/* Rating */}
        {product.rating > 0 && (
          <div className="flex items-center gap-1 mt-2">
            <Star className="size-3 text-app-warning fill-app-warning" />
            <span className="text-xs font-medium text-app-text">{product.rating.toFixed(1)}</span>
