...
export const inngest = new Inngest({ id: "grocery-shop" });
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-low-stock-email", async () => {
      ...
      await sendEmail({
        ...
        html: `<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
...
const sendMonthlyOffers = inngest.createFunction(
  {
    ...
    triggers: [
      cron("0 10 1 * *"), // Every 1st day of the month at 10:00 AM
    ],
  },
  async ({ event, step }) => {
    ...
    if (users.length === 0 || deals.length === 0) {
      return { skipped: true, reason: "No users or deals to send" };
    }
    ...
    for (let i = 0; i < users.length; i += batchSize) {
      ...
      await step.run(`send-offers-batch-${i}`, async () => {
        for (const user of batch) {
          await sendEmail({
            to: user.email,
            subject: `Exclusive Monthly Deals Just for You, ${user.name}!`,
            html: `<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
